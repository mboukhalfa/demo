...
def upload(request):
	...
	if request.method == 'POST':
		...
		for i in range(1,3):
			...
			response = requests.get(WEB_SERVER_URL_ROOT + WEB_SERVER_URL_API_UPLOAD + uuid +'/',headers={'Authorization': 'Bearer '+ oauthAppObject.access_token})
			...
			#  else maybe because of access token or UUID non valide  
				...
				post_data = {'grant_type': 'refresh_token', 'client_id': oauthAppObject.client_id,'client_secret': oauthAppObject.client_secret, 'refresh_token': oauthAppObject.refresh_token}
    ...
    # if a GET 
		...
		if OauthApp.objects.all().count():
			...
			if not request.GET.get('code') :
				...
				post_data = {'code': request.GET["code"], 'redirect_uri':oauthAppObject.redirect_uri , 'grant_type': "authorization_code"}
				...
				response = requests.post(WEB_SERVER_URL_ROOT + '/o/token/', data=post_data,headers={'Authorization': 'Basic '+ bs})
